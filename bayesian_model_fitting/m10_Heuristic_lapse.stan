...
parameters {
  ...
  // -------- group-level SDs ----------
  vector<lower=0,upper=15>[2] sigma_lambda;
  vector<lower=0,upper=15>[2] sigma_bs;
  vector<lower=0,upper=15>[2] sigma_bi;
  vector<lower=0,upper=15>[2] sigma_bc;
  vector<lower=0,upper=15>[2] sigma_br;
  vector<lower=0,upper=0.5>[2] sigma_minp;
  vector<lower=0,upper=0.5>[2] sigma_maxp;
...
model {
  // -------- Hyperpriors (same family as m08) --------
  ...
  for (s in 1:S) {
    for (t in idx_start[s]:idx_end[s]) {
      ...
      real p_core = inv_logit(-lambda[s] * lin);
