...
parameters {
  ...
  vector<lower=0>[2] mu_oumiga;
  vector<lower=0, upper=0.5>[2] mu_minp;
  vector<lower=0, upper=0.5>[2] mu_maxp;
  ...
  vector<lower=0>[2] sigma_kapa;
  vector<lower=0>[2] sigma_etak;
  vector<lower=0>[2] sigma_etaa;
  vector<lower=0>[2] sigma_oumiga;
  vector<lower=0>[2] sigma_minp;
  vector<lower=0>[2] sigma_maxp;
  ...
  vector<lower=0>[S] oumiga;
  vector<lower=0, upper=0.5>[S] minp;
  vector<lower=0, upper=0.5>[S] maxp;
}
...
model {
  ...
  mu_etak   ~ normal(5, 3);
  mu_etaa   ~ normal(5, 3);
  mu_oumiga ~ normal(5, 3);
  mu_minp   ~ normal(0.25, 0.1);
  mu_maxp   ~ normal(0.25, 0.1);
  ...
  sigma_oumiga ~ normal(0, 2);
  sigma_minp   ~ normal(0, 0.1);
  sigma_maxp   ~ normal(0, 0.1);
  ...
  for (s in 1:S) {
    ...
    oumiga[s] ~ normal(mu_oumiga[g], sigma_oumiga[g]);

    minp[s]   ~ normal(mu_minp[g],   sigma_minp[g]);
    maxp[s]   ~ normal(mu_maxp[g],   sigma_maxp[g]);
  }
  ...
  for (s in 1:S) {
    for (t in idx_start[s]:idx_end[s]) {
      ...
      real p_core = inv_logit(lambda[s] * (Uyes - Uno));
      real p = minp[s] + (1 - minp[s] - maxp[s]) * p_core;
